...
const Game: React.FC<GameProps> = ({ gameState, setGameState, stats, setStats, setHp, setSkillCooldowns, onPause, upgrades }) => {
  ...
  const playerRef = useRef<Player>({
    id: 0, x: 50, y: 300, width: 40, height: 60, vx: 0, vy: 0, color: COLORS.PLAYER, hp: 5, maxHp: 5, dead: false,
  ...
  useEffect(() => {
    ...
    // loadSprite('PLAYER', SPRITES.PLAYER); // Commented out to use the new cool code-drawn mech
  ...
  const update = useCallback(() => {
    ...
    if (!player.dead) {
      ...
      if (player.missileTimer > 0) player.missileTimer--;
  ...
  const drawPlayerSprite = (ctx: CanvasRenderingContext2D, p: Player) => {
      ...
      // Strict save/restore block for the entire player draw operation
      ctx.save();
      
      // We will now procedurally draw a cool mech warrior!
      ...
      // Handle crouching (scale Y)
      if (p.isCrouching) ctx.scale(1, 0.6);

      // --- RAGE MODE FLAMES ---
      if (isRage) {
          ctx.save();
          ctx.translate(0, -35); // Above head
          ctx.fillStyle = Math.random() > 0.5 ? '#ff4500' : '#ff0000';
          ctx.shadowBlur = 20;
          ctx.shadowColor = '#ff0000';
          
          // Draw 3 random jagged flames
          for(let i=-1; i<=1; i++) {
      ...
      // Base Colors
      const armorColor = isRage ? '#ff0000' : COLORS.PLAYER; // Pink or Red
      const darkArmor = '#1a1a2e'; // Dark parts
      const jointColor = '#333';
      const glowColor = isRage ? '#ff0000' : '#00ffff';

      // Animation Cycle
      const anim = (Math.abs(p.vx) > 0.1 && !p.isCrouching) ? Math.sin(frameCountRef.current * 0.5) : 0;
      
      // 1. Scarf / Energy Trail (behind)
      if (Math.abs(p.vx) > 2) {
          ctx.save();
          ...
          ctx.beginPath();
          ctx.moveTo(-10, -20);
          ctx.lineTo(-40 - Math.random()*10, -25 + Math.sin(frameCountRef.current)*5);
          ctx.lineTo(-10, -15);
          ctx.fill();
          ctx.restore();
      }
      ...
      // Shin
      ctx.translate(0, 15);
      ctx.rotate(anim * 0.2 + 0.2); // Bent knee
      ctx.fillStyle = armorColor;
      ctx.fillRect(-5, 0, 10, 18); // Boot
      ctx.restore();
      ...
      // 5. Head (Helmet)
      ctx.save();
      ctx.translate(0, -22); // Move to head pos
      // Helmet Base
      ctx.fillStyle = armorColor;
      ctx.beginPath();
      ctx.moveTo(-10, -8);
      ctx.lineTo(10, -8); // Top
      ctx.lineTo(12, 5);  // Jaw right
      ctx.lineTo(0, 10);  // Chin
      ctx.lineTo(-12, 5); // Jaw left
      ctx.closePath();
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.moveTo(-12, -5);
      ctx.lineTo(-15, -15); // Left ear
      ctx.lineTo(-10, -8);
      ctx.fill();
      ctx.restore();

      // 6. Arms & Gun (Drawn on top)
      const gunY = 2; 
      const keys = keysPressed.current;
      
      ctx.save();
      ctx.translate(0, gunY); // Shoulder pivot
      
      let rot = 0;
      // Rotation logic copied from previous simplified version
      if (KEYS.UP.some(k => keys[k]) && !p.isCrouching) {
          rot = -Math.PI/4;
          if (KEYS.LEFT.some(k => !keys[k]) && KEYS.RIGHT.some(k => !keys[k])) {
             rot = Math.PI/4; // Backwards up
          }
      }
      ctx.rotate(rot);
      ...
      // Arm
      ctx.fillStyle = darkArmor;
      ctx.fillRect(-5, -5, 15, 6);
      
      // Gun
      ctx.fillStyle = '#555';
      ctx.fillRect(10, -7, 25, 10); // Barrel
      ctx.fillStyle = '#222';
      ctx.fillRect(10, -2, 12, 8); // Grip/Mag
      
      // Weapon specific flair
      if (p.weaponLevel === WeaponType.RAPID_FIRE) {
          ctx.fillStyle = p.overheated ? '#f00' : '#888';
          ctx.fillRect(35, -8, 8, 12); // Minigun barrel tip
      } else if (p.weaponLevel === WeaponType.LASER) {
          ctx.fillStyle = '#0ff';
          ctx.fillRect(35, -5, 10, 6); // Laser tip
      }
